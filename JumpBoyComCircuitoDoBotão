//Código baseado no código de https://blog.eletrogate.com/jogo-em-lcd-no-arduino/
//Código Jump Boy se o botão estiver pulando sozinho, nesse caso é necessário montar o circuito do botão mostrado no documento.

#include <LiquidCrystal.h>
#define PIN_BUTTON A10 //escolher aqui o pino que será destino ao botão
...
bool drawHero(byte position, char* terrainUpper, char* terrainLower, unsigned int score) {
  ...
  if(fase<10){ lcd.setCursor(14 - digits,0);}
  else{lcd.setCursor(13 - digits,0);}
  lcd.print(fase);
...
void setup(){
  ...
  digitalWrite(PIN_READWRITE, HIGH);
...
void loop(){
  ...
  if (--newTerrainDuration == 0) {
    if (newTerrainType == TERRAIN_EMPTY) {
      ...
      newTerrainDuration = 5 + random(10);
    } else {
