//Código original obtido de AdaaamB, modificado para melhor apresentação.
//Necessita da Font5x3.h para ser utilizado
...
#define SWITCH_PIN  2 // switch pin if enabled - active LOW
OneButton button(2, true); // escolher qual será o pino do botão
...
int dir = 1, prevX = 0, curX = 0, curY = 0, difficulty = 0, len = 3, prevLen = 3, inProgress = 0, isStart = 1, btnActive = 1, btnDelay = 300, verifica = 1;
uint32_t btnLimit = 0;
...
void hitButton() {
  ...
  if (prevX != curX && prevX - prevLen != curX - len) { //if prevX and curX are different, or if prevX - prevLen are different to curX - len -- this is for when we do len--.
    ...
    if (len < 1) { reset(0); difficulty = 0; return; } //invoke reset with win = 0. return to stop further code in this method.
...
{  
  if (inProgress) {
    ...
    if (digitalRead(SWITCH_PIN) == LOW && btnActive && millis() > btnLimit && verifica == 1) {
      verifica = 0;
      btnActive = 0;
